...
public interface EventRepository extends MongoRepository<Event, String> {
  ...
  @Query("{ 'location': ?0, 'eventDate': { $gte: ?1 } , 'isDeleted': false}")
  List<Event> findALlUpcomingEventsByCity(String cityName, LocalDateTime time);

  @Query(
      "{ 'location': ?0, 'eventDate': { $gte: ?1 } , 'isDeleted': false ,  masterCategoryId: ?3}")
